...
if (isset($_REQUEST["flag"])) {
	if ($_REQUEST["flag"]=="show") {
		...
		try {
			...
			$RoomId = querySingleValue($con, "SELECT RoomId From Occupied WHERE OccupiedId=:OccupiedId;",array("OccupiedId"=>$data['OccupiedId']));
			updateRow($con, "UPDATE Room Set ReadyTime=null WHERE RoomId=:RoomId;",array("RoomId"=>$RoomId));
			$con->Commit();
			$response["status"]=1;
			$response["message"]="Ok";
			$response["data"]=$data;
